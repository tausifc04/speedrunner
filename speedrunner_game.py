...
def main():
    global score # access variable score globally
    global leader_names_list
    global leader_scores_list

    # clear lists to prevent overflow
    leader_names_list.clear()
    leader_scores_list.clear()
    ...
    while run:
        ...
        if runner.xcor() >= 595:
            ...
            # start of code: clear current level (walls/paths) and move turtle drawers for walls/paths back to start
            wall_drawer_1.clear()
            wall_drawer_2.clear()
            wall_drawer_1.penup()
            wall_drawer_1.goto(-600, 50)
            ...
            # end of code: clear current level (walls/paths) and move turtle drawers for walls/paths back to start
            # start of code: make turtle blink and wait to prepare user
            runner.hideturtle()
            sleep(0.5)
            runner.showturtle()
            sleep(0.5)
            # end of code: make turtle blink and wait to prepare user
    ...
    if os.path.isfile(lb_file_name) == False: # if leaderboard text file is not found or does not exist
        ...
        manage_lb() # call manage_lb function
    runner.goto(-75, -175)
    runner.write("Press F to restart", font = font_setup)
    wn.onkey(main, "f")
